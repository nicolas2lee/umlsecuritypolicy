...
//!	Generated Date	: Mon, 22, Feb 2016 
	File Path	: DefaultComponent/DefaultConfig/SecureSCADA/Checkpoint.java
*********************************************************************/
...
public class Checkpoint implements RiJStateConcept, Animated {
    ...
    public static AnimClass animClassCheckpoint = new AnimClass("SecureSCADA.Checkpoint",false);
    //#]
    ...
    protected int currentRight;		//## attribute currentRight 
    ...
    //## operation checkTaskAllowed() 
    public void checkTaskAllowed() {
        try {
            animInstance().notifyMethodEntered("checkTaskAllowed",
               new ArgData[] {
               });
        ...
        switch (currentRight){
        	case 1111 :
        		gen(new allow());break;
        	case 1100 :
        		if (currentTask % 10 == 1 || ((currentTask/10)%10==1) ){
        			gen(new notAllow());
        		}else{
        			gen(new allow());
        		}break;                 
        	case 11 :
        		if ((currentTask/100)%10 ==1 || (currentTask/1000)%10 ==1){
        			gen(new notAllow());
        		}else{
        			gen (new allow());
        		}break;
    ...
    //## operation distributeMissions() 
    public void distributeMissions() {
        try {
            animInstance().notifyMethodEntered("distributeMissions",
               new ArgData[] {
               });
        
        //#[ operation distributeMissions() 
        if (currentTask > 11){
        	//need to sendto local                         
        	//System.out.println(getP_check_cen());
        	//getP_check_cen().gen(new sendTaskToField(currentTask/100));  
        	getP_check_cen().gen(new connecttoNet(currentTask/100));
        	//check if need to do task in CC
        	if (currentTask%100 > 0){
        		getP_check_data().gen(new sendTaskToDataserver(currentTask%100));
        	}
        }else{            
        	if(currentTask >0 )
        	getP_check_data().gen(new sendTaskToDataserver(currentTask%100));
    ...
    //## auto_generated 
    public void setCurrentRight(int p_currentRight) {
        currentRight = p_currentRight;
    }
    ...
    public class Reactive extends RiJStateReactive implements AnimatedReactive {
        ...
        public int sentToPolicy_takeEvent(short id) {
            int res = RiJStateReactive.TAKE_EVENT_NOT_CONSUMED;
            if(event.isTypeOf(rejected.rejected_SecureSCADA_id))
                {
                    res = sentToPolicyTakerejected();
                }
            else if(event.isTypeOf(approved.approved_SecureSCADA_id))
                {
        ...
        public int IdentificationRight_takeEvent(short id) {
            int res = RiJStateReactive.TAKE_EVENT_NOT_CONSUMED;
            if(event.isTypeOf(allow.allow_SecureSCADA_id))
                {
                    res = IdentificationRightTakeallow();
                }
            else if(event.isTypeOf(notAllow.notAllow_SecureSCADA_id))
                {
        ...
        public int RightValid_takeEvent(short id) {
            int res = RiJStateReactive.TAKE_EVENT_NOT_CONSUMED;
            if(event.isTypeOf(doneService.doneService_SecureSCADA_id))
                {
                    res = RightValidTakedoneService();
                }
        ...
        public int Idle_takeEvent(short id) {
            int res = RiJStateReactive.TAKE_EVENT_NOT_CONSUMED;
            if(event.isTypeOf(checkLogin.checkLogin_SecureSCADA_id))
                {
        ...
        public int ValidPolicy_takeEvent(short id) {
            int res = RiJStateReactive.TAKE_EVENT_NOT_CONSUMED;
            if(event.isTypeOf(checkRight.checkRight_SecureSCADA_id))
                {
        ...
        //## statechart_method 
        public int sentToCheckRightTakebackRights() {
            backRights params = (backRights) event;
            int res = RiJStateReactive.TAKE_EVENT_NOT_CONSUMED;
            animInstance().notifyTransitionStarted("3");
            sentToCheckRight_exit();
            //#[ transition 3 
            currentRight = params.right;
            checkTaskAllowed();
            //#]
        ...
        public int IdentificationRightTakenotAllow() {
            ...
            IdentificationRight_exit();
            //#[ transition 5 
            getP_check_single().gen(new rejectedRights());
            //#]
            RightInvalid_entDef();
        ...
        //## statechart_method 
        public int RightValidTakedoneService() {
            int res = RiJStateReactive.TAKE_EVENT_NOT_CONSUMED;
            animInstance().notifyTransitionStarted("6");
            RightValid_exit();
            //#[ transition 6 
            getP_check_single().gen(new doneService());
            //#]
            Idle_entDef();
        ...
        public int sentToCheckRight_takeEvent(short id) {
            int res = RiJStateReactive.TAKE_EVENT_NOT_CONSUMED;
            if(event.isTypeOf(backRights.backRights_SecureSCADA_id))
                {
                    res = sentToCheckRightTakebackRights();
                }
    ...
    */
    ...
    public void addAttributes(AnimAttributes msg) {
        ...
        msg.add("currentRight", currentRight);
